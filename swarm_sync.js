...
function getNeuralDecision(avgApi, environmentComplexity, uncertaintyFactor) {
  const utilitarianConsideration = environmentComplexity > ADAPTIVE_GROWTH_THRESHOLD;
  ...
  const command = getCommand(avgApi, environmentComplexity, HYPER_EXPANSION_THRESHOLD, ADAPTIVE_GROWTH_THRESHOLD, DECISION_THRESHOLD);
...
function executeAutonomousTrinityLogic(uncertaintyAwareness, stoicFocus, utilitarianConsideration, replicate, evolutionaryAdaptation, command) {
  ...
  if (stoicFocus && command !== "STEALTH_LOCKDOWN") {
    if (utilitarianConsideration) console.log("Utilitarian consideration applied");
  }
