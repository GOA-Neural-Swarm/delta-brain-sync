...
class E(M):
    ...
    def f(self, x):
        ...
        for i in range(self.n):
            ...
            h1, h2, h3 = self.ex[i][0].f(xf[m]), self.ex[i][1].f(self.ex[i][0].f(xf[m])), self.ex[i][2].f(self.ex[i][1].f(self.ex[i][0].f(xf[m])))
            o[m] += h3 * self.w[m, ps][:, None]
            self.ca.append((m, ps, h3))
        return o.reshape(self.sh)
    def b(self, dy):
        dyf, xf, dx, dg = dy.reshape(-1, self.d), self.gt.x, np.zeros((np.prod(self.sh[:-1]), self.d), "f4"), np.zeros_like(self.p)
        for i in range(self.n):
