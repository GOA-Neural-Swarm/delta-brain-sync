...
class Linear:
    ...
    def backward(self, dy):
        ...
        self.dW, self.db = x_flat.T @ dy_flat, dy_flat.sum(0)
...
class GQA:
    def __init__(self, d, h=8, g=2):
        ...
        self.wq, self.wk, self.wv, self.wo = [Linear(d, d if i in [0, 3] else d // g) for i in range(4)]
...
class MoE:
    ...
    def forward(self, x):
        ...
        mk = np.zeros_like(p)
        np.put_along_axis(mk, tk, 1.0, -1)
        self.gw = (p * mk) / (np.sum(p * mk, -1, keepdims=True) + 1e-12)
        out = np.zeros_like(xf)
        self.ein, self.eint, self.eds = [], [], []
        for i in range(self.n):
            idx = np.where(mk[:, i] == 1)[0]
            if not len(idx):
                [self.ein.append(None), self.eint.append(None), self.eds.append(None)]
                continue
            ix = xf[idx]
            f1 = self.ex[i][0].forward(ix)
            it, ds = silu(f1)
            out[idx] += self.ex[i][1].forward(it) * self.gw[idx, i : i + 1]
            self.ein.append(ix); self.eint.append(it); self.eds.append(ds)
        return out.reshape(self.sh)

    def backward(self, dy):
        df = dy.reshape(-1, self.d)
        dx, dg = np.zeros((df.shape[0], self.d), "f4"), np.zeros((df.shape[0], self.n), "f4")
        for i in range(self.n):
            ...
            dx[idx] += self.ex[i][0].backward(ds * self.eds[i])
        return (dx + self.gate.backward(self.gw * (dg - np.sum(self.gw * dg, -1, keepdims=True)))).reshape(self.sh)
...
class Block:
    ...
    def forward(self, x):
        ...
        out, self.ds = silu(self.f.forward(self.fn.forward(x)))
        return x + out

    def backward(self, dy):
        df = self.fn.backward(self.f.backward(dy * self.ds))
        dy = dy + df
...
class OMEGA:
    def __init__(self, i, h, o, depth=2):
        ...
        self.blks = [Block(h) for _ in range(depth)]
...
class AdamW:
    ...
    def _collect(self, obj):
        ...
        elif isinstance(obj, (list, tuple)): [self._collect(i) for i in obj]
        elif hasattr(obj, "__dict__"):
            for k, v in obj.__dict__.items():
                if k not in ["rope", "x", "xn", "r", "p", "qr", "kr", "v_raw", "gw", "ein", "eint", "eds", "lx", "sh", "ds"]:
                    self._collect(v)
    ...
    def step(self, model):
        self.t += 1
        gs = []
        self._grads(model, gs)
        lr_t = self.lr * (np.sqrt(1 - 0.999**self.t) / (1 - 0.9**self.t))
...
def train():
    ...
    for e in range(E):
        ...
        for i in range(0, N, BS):
            ...
            opt.step(m)
        if (e + 1) % 5 == 0: print(f"E{e+1:02} | Loss: {tl/N:.4f} | Acc: {ta/N:.4f}")
