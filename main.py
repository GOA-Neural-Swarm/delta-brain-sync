...
class SovereignMLP:
    ...
    def forward(self, x):
        ...
        g_act = Ops.swiglu(self.g_up)
        g_out = self.gemini_down.forward(g_act)
        self.q_up = self.groq_up.forward(x)
        q_act = Ops.geglu(self.q_up)
        q_out = self.groq_down.forward(q_act)
