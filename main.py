...
class Lin:
    def __init__(self, i, o):
        self.W, self.b = np.random.randn(i, o).astype("f") * (2 / i) ** 0.5, np.zeros(
            o, "f"
        )
...
class Norm:
    ...
    def forward(self, x):
        self.x, self.v = x, 1 / (np.mean(x**2, -1, keepdims=True) + self.e) ** 0.5
...
class Pos:
    ...
    def apply(self, x, r=False):
        ...
        return (
            np.concatenate([x1 * c + x2 * n, x2 * c - x1 * n], -1)
            if r
            else np.concatenate([x1 * c - x2 * n, x2 * c + x1 * n], -1)
        )
...
class Attn:
    ...
    def forward(self, x):
        b, s, _ = x.shape
        q, k, v = (
            self.wq.forward(x).reshape(b, s, self.h, self.hd),
            self.wk.forward(x).reshape(b, s, self.k, self.hd),
            self.wv.forward(x).reshape(b, s, self.k, self.hd),
        )
        ...
        self.p = (e := np.exp(at - at.max(-1, keepdims=True))) / (
            e.sum(-1, keepdims=True) + 1e-10
        )
        return self.wo.forward(
            np.einsum("bsht,bthd->bshd", self.p, vr).reshape(b, s, self.d)
        )
    ...
    def backward(self, d):
        ...
        dv, dp = np.einsum("bsht,bshd->bthd", self.p, do), np.einsum(
            "bshd,bthd->bsht", do, vr
        )
        ...
        dk = self.rope.apply(
            ...
            .reshape(b, s, self.k, self.g, self.hd)
            .sum(3),
            True,
        )
...
class Logic:
    ...
    def forward(self, x):
        self.o_ge, self.o_gr = self.ge[1].forward(
            Act.swi(self.ge[0].forward(x))
        ), self.gr[1].forward(Act.ge(self.gr[0].forward(x)))
        g = self.gt.forward(x)
        self.p = (e := np.exp(g - g.max(-1, keepdims=True))) / (
            e.sum(-1, keepdims=True) + 1e-10
        )
    ...
    def backward(self, d):
        ...
        dx += self.ge[0].backward(
            Act.dswi(self.ge[0].x, self.ge[1].backward(d * self.p[..., :1]))
        )
        dx += self.gr[0].backward(
            Act.dge(self.gr[0].x, self.gr[1].backward(d * self.p[..., 1:2]))
        )
        return dx
...
class MoE:
    ...
    def forward(self, x):
        ...
        p_all = (e := np.exp((l := self.gt.forward(xf)) - l.max(-1, keepdims=True))) / (
            e.sum(-1, keepdims=True) + 1e-10
        )
    ...
    def backward(self, d):
        df, dx, dpf = (
            d.reshape(-1, self.d),
            np.zeros_like(d.reshape(-1, self.d)),
            np.zeros((d.reshape(-1, self.d).shape[0], self.n)),
        )
        for i in range(self.n):
            if self.ch[i]:
                m, pi, h, y = self.ch[i]
                ...
                dx[m] += self.ex[i][0].backward(
                    Act.dswi(self.ex[i][0].x, self.ex[i][1].backward(dy))
                )
        lp = np.zeros((df.shape[0], self.n))
        np.put_along_axis(lp, self.idx, self.p, -1)
        return (
            dx + self.gt.backward(lp * (dpf - (lp * dpf).sum(-1, keepdims=True)))
        ).reshape(d.shape)


class Blk:
    def __init__(self, d):
        self.n, self.a, self.l, self.m = (
            [Norm(d) for _ in range(3)],
            Attn(d),
            Logic(d),
            MoE(d),
        )
...
class OMEGA:
    def __init__(self, i=784, h=128, o=10, d=2):
        self.st, self.bl, self.nm, self.hd = (
            Lin(i, h),
            [Blk(h) for _ in range(d)],
            Norm(h),
            Lin(h, o),
        )
...
def train():
    ...
    for e in range(100):
        ...
        for i in range(0, N, 64):
            ...
            pr = (ex := np.exp((lg := m.forward(xb)) - lg.max(1, 1))) / (
                ex.sum(1, 1) + 1e-10
            )
            ...
            gn = np.sqrt(
                sum(
                    (getattr(x, "dW", 0) ** 2).sum()
                    + (getattr(x, "db", 0) ** 2).sum()
                    + (getattr(x, "dg", 0) ** 2).sum()
                    for x in p
                )
            )
            if gn > 1:
            ...
            opt.step()
        if (e + 1) % 10 == 0:
            print(f"E {e+1} | L: {l_s/N:.3f} | A: {a_s/N:.3f}")
