...
class Lin:
    def __init__(self, i, o):
        self.W, self.b = np.random.randn(i, o).astype("f") * (2/i)**0.5, np.zeros(o, "f")
...
class Attn:
    ...
    def backward(self, d):
        ...
        dk = self.rope.apply(np.einsum("bsht,bthd->bthd", da, self.q).reshape(b, s, self.k, self.g, self.hd).sum(3), True)
...
class Blk:
    def __init__(self, d):
        self.n, self.a, self.l, self.m = [Norm(d) for _ in range(3)], Attn(d), Logic(d), MoE(d)
...
class OMEGA:
    ...
    def forward(self, x):
        ...
        x = x + self.gemini.forward(x)
        x = x + self.groq.forward(x)
        return self.hd.forward(self.nm.forward(x[:, 0, :]))

    def backward(self, d):
        d = self.nm.backward(self.hd.backward(d))[:, None, :]
        d = d + self.gemini.backward(d)
        d = d + self.groq.backward(d)
