...
def silu(x):
    return x / (1.0 + np.exp(-np.clip(x, -15, 15)))
...
class GQA:
    def __init__(self, d, h=8, g=2):
        ...
        self.wq, self.wk, self.wv, self.wo = [
            Linear(d, d if i == 0 or i == 3 else d // g) for i in range(4)
        ]
    ...
    def forward(self, x):
        ...
        return self.wo.forward(
            np.einsum("bsht,bthd->bshd", self.p, ve).reshape(b, s, -1)
        )
    ...
    def backward(self, dy):
        ...
        dq, dk = self.rope.apply(dqr, 1).reshape(b, s, -1), self.rope.apply(
            dkr, 1
        ).reshape(b, s, -1)
        return (
            self.wq.backward(dq)
...
class MoE:
    ...
    def forward(self, x):
        ...
        for i in range(self.n):
            ...
            if not len(idx):
                self.ein.append(None)
                self.eint.append(None)
                continue
            ix = xf[idx]
            it = silu(self.ex[i][0].forward(ix))
...
class AdamW:
    ...
    def _get(self, obj):
        if isinstance(obj, Linear):
            self.p.extend([obj.W, obj.b])
        elif isinstance(obj, RMS):
            self.p.append(obj.g)
        elif isinstance(obj, (list, tuple)):
            [self._get(i) for i in obj]
...
def train():
    ...
    for e in range(E):
        ...
        for i in range(0, N, BS):
            ...
            opt.step(m)
        if (e + 1) % 5 == 0:
            print(f"E{e+1} | Loss: {l/N:.4f} | Acc: {a/N:.4f}")
