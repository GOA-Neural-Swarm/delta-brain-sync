...
class GeminiAttention(Module):
    ...
    def b(self, dy):
        ...
        dqr, dkr = np.einsum("bsht,bthd->bshd", ds, self.kr), np.einsum("bsht,bshd->bthd", ds, self.qr)
...
class GroqMoE(Module):
    ...
    def f(self, x):
        ...
        for i in range(self.n):
            ...
            if not np.any(mask): self.cache.append(None); continue
...
class ConsensusBlock(Module):
    ...
    def f(self, x):
        ...
        self.p = (e := np.exp(gl - gl.max(-1, keepdims=True))) / (e.sum(-1, keepdims=True) + 1e-9)
        return x + self.p[:, 0:1, None] * self.og + self.p[:, 1:2, None] * self.oq

    def b(self, dy):
        dgl = np.zeros_like(self.p)
        dgl[:, 0], dgl[:, 1] = np.sum(dy * self.og, axis=(1, 2)), np.sum(dy * self.oq, axis=(1, 2))
