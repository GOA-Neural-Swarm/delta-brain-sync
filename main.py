...
class GeminiExpert:
    ...
    def forward(self, x):
        ...
        self.combined = self.act_x1 * self.x2
        return self.w3.forward(self.combined)

    def backward(self, dout):
        d_comb = self.w3.backward(dout)
        dx1 = d_comb * self.x2 * d_silu(self.x1)
        dx2 = d_comb * self.act_x1
        return self.w1.backward(dx1) + self.w2.backward(dx2)

class GroqExpert:
    def __init__(self, dim):
        self.w1 = Linear(dim, dim * 4)
        self.w2 = Linear(dim * 4, dim)

    def forward(self, x):
        self.z = silu(self.w1.forward(x))
        return self.w2.forward(self.z)

    def backward(self, dout):
        dz = self.w2.backward(dout) * d_silu(self.w1.x)
        return self.w1.backward(dz)

class SovereignMoE:
    def __init__(self, dim):
        self.gemini = GeminiExpert(dim)
        self.groq = GroqExpert(dim)
...
def train():
    ...
    model = SovereignArchitectV10(h_d=128, out_d=10, depth=4)
    params = model.get_params()
    opt = LionOptimizer(params, lr=1e-4, wd=0.01)
    bs, epochs = 64, 40
    
    print("OMEGA-ASI | V10-SOVEREIGN-CORE | RECURSIVE EVOLUTION")
    ...
    for ep in range(epochs):
        ...
        for i in range(0, len(X), bs):
            ...
            gn = 0
            for p in params:
                if hasattr(p, "dW"): gn += np.sum(p.dW**2) + np.sum(p.db**2)
                elif hasattr(p, "dg"): gn += np.sum(p.dg**2)
            gn = np.sqrt(gn)
            if gn > 1.0:
                for p in params:
                    if hasattr(p, "dW"): p.dW /= gn; p.db /= gn
                    elif hasattr(p, "dg"): p.dg /= gn
