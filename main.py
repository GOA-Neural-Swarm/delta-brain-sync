...
class L:
    def __init__(self, i, o):
        s = (2 / (i + o))**0.5
        self.w, self.b = T(np.random.randn(i, o) * s), T(np.zeros(o))
    def f(self, x):
        self.x = x
...
class N:
    def __init__(self, d):
        self.g, self.e = T(np.ones(d)), 1e-6
    def f(self, x):
        self.x = x
...
class A:
    ...
    def f(self, x):
        ...
        self.qr, self.kr, self.vr = self._rp(q, co, si), self._rp(k, co, si), v
        ke, ve = np.repeat(self.kr, self.g, 2), np.repeat(v, self.g, 2)
        at = np.einsum("bshd,bthd->bsht", self.qr, ke) * self.s
    ...
    def bk(self, dy):
        b, s = dy.shape[:2]
        dw = self.wo.bk(dy).reshape(b, s, self.h, self.hd)
        ke, ve = np.repeat(self.kr, self.g, 2), np.repeat(self.vr, self.g, 2)
        dp = np.einsum("bshd,bthd->bsht", dw, ve)
        da = self.p * (dp - np.sum(self.p * dp, -1, keepdims=1)) * self.s
        ...
        return self.wq.bk(dqr.reshape(b, s, -1)) + self.wk.bk(dk.reshape(b, s, -1)) + self.wv.bk(dv.reshape(b, s, -1))

class M:
    def __init__(self, d, n=4, k=2):
        self.d, self.n, self.k = d, n, k
        self.gt = L(d, n)
        self.w1 = [T(np.random.randn(d, d * 2) * (2 / (d * 3))**0.5) for _ in range(n)]
        self.w2 = [T(np.random.randn(d * 2, d) * d**-0.5) for _ in range(n)]
    ...
    def bk(self, dy):
        ...
        for i in range(self.n):
            ...
            self.w2[i].g += at.T @ di
            da = self.sw[i].bk(di @ self.w2[i].d.T)
            self.w1[i].g += xi.T @ da
            dx[m] += da @ self.w1[i].d.T
        return (dx + self.gt.bk(dg - np.mean(dg, -1, keepdims=1))).reshape(dy.shape)

class B:
    def __init__(self, d):
        self.n1, self.at, self.n2, self.me = N(d), A(d), N(d), M(d)
        self.p = T(np.array([0.5]))
    def f(self, x):
    ...
    def bk(self, dy):
        ...
        dx1 = dy * (1 - self.p.d) + self.n2.bk(self.me.bk(dy * self.p.d))
        return dx1 + self.n1.bk(self.at.bk(dx1))

class Model:
    def __init__(self, di, dm, do, dep=2):
        self.emb, self.bl = L(di, dm), [B(dm) for _ in range(dep)]
        self.fn, self.hd = N(dm), L(dm, do)
        self.ps = []
        self._gp(self)
    def _gp(self, o):
        if isinstance(o, T): self.ps.append(o)
        elif hasattr(o, "__dict__"):
            for v in o.__dict__.values():
                if isinstance(v, list): [self._gp(i) for i in v]
                else: self._gp(v)
    def f(self, x):
    ...
    def bk(self, dy):
        dy = self.fn.bk(self.hd.bk(dy))
        db = np.zeros((dy.shape[0], self.emb.x.shape[1], dy.shape[1]), "f4")
        db[:, -1, :] = dy
        for b in reversed(self.bl): db = b.bk(db)
        self.emb.bk(db)
...
class Opt:
    ...
    def step(self):
        self.t += 1
        lt = self.lr * ((1 - 0.999**self.t)**0.5 / (1 - 0.9**self.t))
...
def train():
    N_S, D, C, BS, E = 1024, 784, 10, 64, 50
    X, Y = np.random.randn(N_S, D).astype("f4"), np.random.randint(0, C, N_S)
    m = Model(D, 128, C)
    opt = Opt(m.ps, 3e-3)
    for e in range(E):
        idx, ls, ac = np.random.permutation(N_S), [], []
        for i in range(0, N_S, BS):
            xb, yb = X[idx[i:i+BS]], Y[idx[i:i+BS]]
            lg = m.f(xb)
            pr = np.exp(lg - np.max(lg, -1, keepdims=1))
