...
class T:
  def __init__(self, d): self.d, self.g = d.astype("f4"), n.zeros_like(d, "f4")
...
class N(M):
  def __init__(self, d, e=1e-6): self.g, self.e = T(n.ones(d)), e
  def f(self, x):
    self.x, self.m, self.v = x, x.mean(-1, 1), x.var(-1, 1)
    self.r = 1 / n.sqrt(self.v + self.e)
    return self.g.d * (x - self.m) * self.r
  def b(self, dy):
    xn, dxn = (self.x - self.m) * self.r, dy * self.g.d
...
class R(M):
  ...
  def apply(self, x, v=0):
    s = x.shape[1]
    c, sn, r, i = self.c[:s, None, :], self.s[:s, None, :], x[..., ::2], x[..., 1::2]
    o = n.empty_like(x)
...
class A(M):
  ...
  def b(self, dy):
    ...
    ds = self.p_ * (dp - (self.p_ * dp).sum(-1, 1)) * (self.hd**-0.5)
...
class E(M):
  ...
  def f(self, x):
    ...
    for i in range(self.n):
      ...
      ps, h = n.where(self.ix[m] == i)[1], xf[m]
      hs = [h]
  ...
  def b(self, dy):
    ...
    for i in range(self.n):
      if self.ca[i]:
        ...
        dx[m] += dh
    return (dx + self.gt.b(self.p_ * (dg - (self.p_ * dg).sum(-1, 1)))).reshape(self.sh)
...
class Opt:
  def __init__(self, p, lr=1e-3, b1=.9, b2=.999, wd=.01):
    ...
    self.m, self.v = [n.zeros_like(x.d) for x in self.p], [n.zeros_like(x.d) for x in self.p]
  def step(self):
    self.t += 1
...
def train():
  ...
  m, o = Net(D, 128, C), Opt(Net(D, 128, C).p(), 1e-3, wd=.1)
  o.p = list(m.p())
  for e in range(EP):
    ix, ls = n.random.permutation(N_), []
    for i in range(0, N_, BS):
      xb, yb = X[ix[i:i+BS]], Y[ix[i:i+BS]]
      p = sm(m.f(xb))
      ls.append([-n.mean(n.log(p[n.arange(len(yb)), yb] + 1e-9)), n.mean(p.argmax(1) == yb)])
      dl = p.copy(); dl[n.arange(len(yb)), yb] -= 1
      m.b(dl / len(yb)); o.step()
    if (e + 1) % 5 == 0: print(f"E {e+1:03} | L: {n.mean(ls,0)[0]:.4f} | A: {n.mean(ls,0)[1]:.4f}")
