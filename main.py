...
class Linear(Module):
    def __init__(self, i, o, bias=True):
        scale = np.sqrt(2.0 / i)
        self.w = Tensor(np.random.standard_normal((i, o)) * scale)
        self.b_vec = Tensor(np.zeros(o)) if bias else None

    def f(self, x):
        self.x = x
        return x @ self.w.data + (self.b_vec.data if self.b_vec else 0)
...
class SwiGLU(Module):
    def f(self, x):
        ...
        self.sig = 1.0 / (1.0 + np.exp(-np.clip(x1, -12, 12)))
...
class GeminiAttention(Module):
    ...
    def f(self, x):
        ...
        self.qr, self.kr = self._rope(q), self._rope(k)
        self.v = v
        sc = np.einsum("bshd,bthd->bsht", self.qr, self.kr) * (self.hd**-0.5)
        self.p = (e := np.exp(sc - sc.max(-1, keepdims=True))) / (e.sum(-1, keepdims=True) + 1e-12)
    ...
    def b(self, dy):
        ...
        dqr = np.einsum("bsht,bthd->bshd", ds, self.kr)
        dkr = np.einsum("bsht,bshd->bthd", ds, self.qr)
        dv = np.einsum("bsht,bshd->bthd", self.p, do)
        dq = self._rope(dqr, True)
        dk = self._rope(dkr, True)
...
class RedundantConsensusBlock(Module):
    def __init__(self, d):
        ...
        self.gemini = GeminiAttention(d)
        self.groq = GroqMoE(d)
        self.gate = Linear(d, 2, False)

    def f(self, x):
        self.x = x
        self.og = self.gemini.f(self.n1.f(x))
        self.oq = self.groq.f(self.n2.f(x))
        gl = self.gate.f(np.mean(x, axis=1))
        self.p = (e := np.exp(gl - gl.max(-1, keepdims=True))) / (e.sum(-1, keepdims=True) + 1e-12)
        return x + self.p[:, 0:1, None] * self.og + self.p[:, 1:2, None] * self.oq

    def b(self, dy):
        dgl = np.zeros_like(self.p)
        dgl[:, 0] = np.sum(dy * self.og, axis=(1, 2))
        dgl[:, 1] = np.sum(dy * self.oq, axis=(1, 2))
