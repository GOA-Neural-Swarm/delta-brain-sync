...
class A:
    ...
    def b(self, dy):
        ...
        dq, dk = np.einsum("bsht,bthd->bshd", da, ke), np.einsum(
            "bsht,bshd->bthd", da, self.qr
        ).reshape(b, s, self.h // self.g, self.g, self.hd).sum(3)
        dv = (
...
class M:
    def __init__(self, d, n=4, k=2):
        ...
        self.gt = L(d, n)
        self.w1 = [
            P(np.random.randn(d, d * 2) * (2 / (d * 3)) ** 0.5) for _ in range(n)
        ]
        self.w2 = [P(np.random.randn(d * 2, d) * d**-0.5) for _ in range(n)]
