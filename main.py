...
class ME:
    def __init__(self, d, n=4, k=2):
        ...
        self.gate = L(d, n)
        self.w1 = [
            T(np.random.normal(0, np.sqrt(2 / (d + d * 2)), (d, d * 2)))
            for _ in range(n)
        ]
...
class Opt:
    def __init__(self, ps, lr=1e-3, wd=0.01):
        ...
        self.m, self.v = [np.zeros_like(p.d) for p in ps], [
            np.zeros_like(p.d) for p in ps
        ]
...
def train():
    ...
    m, opt = Model(D, 128, C), Opt(
        Model(D, 128, C).ps, 3e-3
