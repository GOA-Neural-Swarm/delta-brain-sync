...
class SG(M):
    ...
    def b(self, dy):
        ds, dv = dy * self.v, dy * (self.g * self.sig)
...
class GQA(M):
    ...
    def b(self, dy):
        b, s = dy.shape[:2]
        dyw = self.wo.b(dy).reshape(b, s, self.h, self.hd)
        dp = np.einsum("bshd,bthd->bsht", dyw, np.repeat(self.v_o, self.g, 2))
        ...
        dv = np.einsum("bsht,bshd->bthd", self.p, dyw).reshape(b, s, -1, self.g, self.hd).sum(3)
...
class DSL(M):
    def __init__(self, d):
        self.p1, self.p2, self.fuz = [L(d, d*2), SG(), L(d, d)], [L(d, d*2), SG(), L(d, d)], L(d*2, d)
    def f(self, x):
...
class B(M):
    def __init__(self, d):
        self.n1, self.at, self.n2, self.ds, self.n3, self.mo = RN(d), GQA(d), RN(d), DSL(d), RN(d), RE(d)
    def f(self, x):
...
if __name__ == "__main__":
    ...
    for e in range(E):
        ...
        for i in range(0, N, BS):
            ...
            lg = m.f(xb)
            pr = (ex := np.exp(lg - lg.max(-1, 1, keepdims=True))) / (ex.sum(-1, keepdims=True) + 1e-12)
        ...
        if (e + 1) % 5 == 0: print(f"E {e+1:02} | L: {np.mean(ls):.4f} | A: {np.mean(ac):.4f}")