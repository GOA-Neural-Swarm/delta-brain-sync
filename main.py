...
class GeminiAttention(Module):
    ...
    def b(self, dy):
        ...
        dq, dk = np.einsum("bsht,bthd->bshd", ds, self.k), np.einsum(
            "bsht,bshd->bthd", ds, self.q
        )
...
class HybridBlock(Module):
    ...
    def f(self, x):
        ...
        self.g = (e := np.exp(gl := self.fuser.f(np.mean(x, 1)))) / (
            e.sum(-1, keepdims=True) + 1e-9
        )
    ...
    def b(self, dy):
        ...
        dg[:, 0], dg[:, 1] = np.sum(dy * self.a_out, (1, 2)), np.sum(
            dy * self.m_out, (1, 2)
        )
...
def train():
    ...
    for e in range(E):
        ...
        for i in range(0, N, BS):
            ...
            logits = model.f(xb)
            probs = (ex := np.exp(logits - np.max(logits, -1, keepdims=True))) / (
                ex.sum(-1, keepdims=True) + 1e-9
            )
