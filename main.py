...
def query_groq_api(prompt):
    ...
    # os.getenv အစား မငျးရဲ့ GH_TOKEN လိုမြိုး secret ထဲက ယူတာ ပိုသခြောပါတယျ
...
def broadcast_to_swarm(command, gen_version):
    """
    sub-node-logic/instruction.json ကို update လုပျပွီး 
    Swarm တဈခုလုံးကို ညှှနျကွားခကြျအသဈ လှမျးပို့သညျ။
    """
    if not GH_TOKEN:
        print("⚠️ [BROADCAST]: GH_TOKEN missing. Broadcast skipped.")
        return

    # sub-node တှေ fetch လုပျမယ့ျ instruction repo
...
def autonomous_git_push(gen, thought, modified_files):
    ...
    try:
        ...
        if os.path.exists(REPO_PATH):
            try:
                # Shutil ဖွင့ျ အရငျကွိုးစားဖကြျမညျ
                shutil.rmtree(REPO_PATH)
            except Exception:
                # မရပါက OS Level Force Command ဖွင့ျ ဖကြျမညျ
        ...
        # Token ပါဝငျသော Remote URL ကို တညျဆောကျခွငျး
        ...
        # GitPython ကိုသုံး၍ Fresh Clone လုပျခွငျး
        repo = git.Repo.clone_from(remote_url, REPO_PATH)
        print("✅ [STEP 2]: Remote Assets Acquired.")

        # --- 🔱 THE HYBRID BRIDGE: INTERNAL GIT NEUTRALIZATION ---
        # Clone ထဲက .git ကို ဖကြျမှသာ သင့ျရဲ့ Manual Init Logic အလုပျလုပျမှာပါ
        ...
        os.chdir(REPO_PATH) # Repo folder ထဲသို့ ဝငျရောကျခွငျး
        
        # Shell command မြားဖွင့ျ manual ပွနျစခွငျး (Your Original Flow)
        os.system("git init")
        ...
        # မူလ directory သို့ ပွနျထှကျ၍ ပွငျဆငျထားသော file မြားကို copy ကူးခွငျး
        os.chdir(original_cwd)
        
        # [OMNI-LOGIC]: AI ပွငျလိုကျသော file အားလုံးကို list ထဲကနေ တဈခုခငြျး copy ကူးမညျ
        injected_count = 0
        if modified_files:
        ...
        # [CORE-PROTECTION]: သငျသတျမှတျခဲ့သော အဓိက file ၃ ခုကို အမွဲ copy ကူးခွငျး
        static_targets = ["main.py", "brain.py", "ai_experiment.py"]
        for file in static_targets:
            # AI မပွငျလိုကျသောျလညျး လကျရှိ folder ထဲမှာရှိနရေငျ Update ဖွဈအောငျ ကူးမညျ
        ...
        os.chdir(REPO_PATH) # Push ရနျအတှကျ repo folder ထဲ ပွနျဝငျခွငျး
        
        # Changes အားလုံးကို Stage လုပျခွငျး
        os.system("git add .")
        
        # Change ရှိမရှိ check ပွီး commit လုပျခွငျး
        status_check = os.popen("git status --porcelain").read().strip()
        if status_check:
            commit_msg = f"🧬 Gen {gen} Hyper-Evolution [skip ci]"
            os.system(f'git commit -m "{commit_msg}"')
            
            # FORCE PUSH: Remote repository ကို အကွှငျးမဲ့ လှှမျးမိုးခွငျး
        ...
        # အလုပျပွီးလြှငျ မူလ directory သို့ ပွနျသှားခွငျး
        os.chdir(original_cwd)

    except Exception as e:
        print(f"❌ [CRITICAL GIT ERROR]: {e}")
        # Error တကျပါက သင့ျရဲ့ Original Rollback ကို ခေါျမညျ
        if is_code_update:
...
        
        # 🔱 [SWARM TRIGGER]: Logic အသဈရှိရငျ EVOLVE ခိုငျးမယျ၊ မရှိရငျ SYNC ပဲ လုပျမယျ
