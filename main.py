...
class N:
 ...
 def __call__(self,x):self.n=(x-x.mean(0))/np.sqrt(x.var(0)+1e-5);return self.n*self.g+self.b
...
class M:
 def __init__(self,i=784,h=128,o=10,c=3):
  ...
  self.m=[np.zeros_like(getattr(s,a))for s,a in self.p];self.v=[x*0 for x in self.m]
 def forward(self,x):
 ...
 def backward(self,d):
  ...
  for i,(s,a)in enumerate(self.p):
   g=getattr(s,'d'+a if a!='g' else'dg');self.m[i]=.9*self.m[i]+.1*g;self.v[i]=.999*self.v[i]+.001*g**2
...
m=M()
for e in range(101):
 z=m.forward(X);v=np.exp(z-z.max(1,keepdims=1));p=v/v.sum(1,keepdims=1)
 l,d=-np.log(p[range(100),Y]+1e-9).mean(),p.copy();d[range(100),Y]-=1;m.backward(d)
