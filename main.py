...
class SovereignFusion:
    def __init__(self, d):
        self.gemini = GeminiGQA(d)
        self.groq = GroqMoE(d)
        self.mix = Linear(d, 2)
    def forward(self, x):
        self.og, self.oq = self.gemini.forward(x), self.groq.forward(x)
        self.m_lg = self.mix.forward(x)
        ex_m = np.exp(self.m_lg - self.m_lg.max(-1, keepdims=True))
        self.mp = ex_m / ex_m.sum(-1, keepdims=True)
        return self.mp[..., 0:1] * self.og + self.mp[..., 1:2] * self.oq
    def backward(self, dy):
        dg, dq = dy * self.mp[..., 0:1], dy * self.mp[..., 1:2]
        dmg = (dy * self.og).sum(-1, keepdims=True)
        dmq = (dy * self.oq).sum(-1, keepdims=True)
        dm = np.concatenate([dmg, dmq], axis=-1)
        dml = self.mp * (dm - (self.mp * dm).sum(-1, keepdims=True))
        return self.gemini.backward(dg) + self.groq.backward(dq) + self.mix.backward(dml)

class SovereignBlock:
    def __init__(self, d):
        self.n1, self.f1, self.n2, self.f2 = RMSNorm(d), SovereignFusion(d), RMSNorm(d), GroqMoE(d)
    def forward(self, x):
