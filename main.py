...
def step(m,x,y):
 z=m.f(x);p=(e:=np.exp(z-z.max(1,**K)))/e.sum(1,**K);j=np.arange(len(y));ls=-np.log(p[j,y]+1e-10).mean();dl=p.copy();dl[j,y]-=1;m.b(dl/len(y));return ls
X,Y,m=np.random.randn(100,784).astype('f4'),np.random.randint(10,size=100),M()
for e in range(101):
 ls=step(m,X,Y)
 if not e%10:print(f"E:{e:03}|L:{ls:.4f}|A:{(m.f(X).argmax(1)==Y).mean():.4f}")