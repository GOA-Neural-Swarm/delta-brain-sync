...
class Tensor:
    def __init__(self, d):
        ...
        self.grad = np.zeros_like(d, dtype="f4")
...
class Linear:
    ...
    def backward(self, dy):
        ...
        self.w.grad += self.x.reshape(-1, self.x.shape[-1]).T @ dx
        self.b.grad += dx.sum(0)
...
class GQA:
    ...
    def forward(self, x):
        ...
        def rope(t):
            r, i = t[..., ::2], t[..., 1::2]
            return np.stack([r*co[:,None,:] - i*si[:,None,:], r*si[:,None,:] + i*co[:,None,:]], -1).reshape(t.shape)
        
        self.qr, self.kr, self.vr = rope(q), rope(k), v
        kr, vr = np.repeat(self.qr, self.g, 2), np.repeat(v, self.g, 2)
        at = np.einsum("bshd,bthd->bsht", self.qr, np.repeat(self.kr, self.g, 2)) * self.scale
        at = np.exp(at - np.max(at, -1, keepdims=True))
        self.p = at / (np.sum(at, -1, keepdims=True) + 1e-12)
        out = np.einsum("bsht,bthd->bshd", self.p, vr).reshape(b, s, -1)
        return self.wo.forward(out)

    def backward(self, dy):
        b, s = dy.shape[:2]
        dw = self.wo.backward(dy).reshape(b, s, self.h, self.hd)
        vr = np.repeat(self.vr, self.g, 2)
        dp = np.einsum("bshd,bthd->bsht", dw, vr)
        ...
        return self.wq.backward(dqr.reshape(b, s, -1)) + self.wk.backward(dkr.reshape(b, s, -1)) + self.wv.backward(dv.reshape(b, s, -1))
...
class MoE:
    ...
    def backward(self, dy):
        ...
        return (dx + self.gate.backward(dg - np.mean(dg, -1, keepdims=True))).reshape(dy.shape)

class SovereignBlock:
    def __init__(self, d):
        self.n1, self.at, self.n2, self.moe, self.pg = RMSNorm(d), GQA(d), RMSNorm(d), MoE(d), Tensor(np.array([0.5]))
...
class OMEGA_ASI:
    def __init__(self, di, dm, do, depth=2):
        self.emb, self.blks, self.fn, self.hd = Linear(di, dm), [SovereignBlock(dm) for _ in range(depth)], RMSNorm(dm), Linear(dm, do)
        self.params = self._get_p()

    def _get_p(self):
        p = []
        def walk(o):
            if hasattr(o, 'w') and isinstance(o.w, Tensor): p.extend([o.w, o.b])
            elif hasattr(o, 'g') and isinstance(o.g, Tensor): p.append(o.g)
    ...
    def backward(self, dy):
        ...
        db[:, -1, :] = dy
        for b in reversed(self.blks): db = b.backward(db)
        self.emb.backward(db)

class AdamW:
    def __init__(self, ps, lr=1e-3, wd=0.01):
        self.ps, self.lr, self.wd = ps, lr, wd
        self.m = [np.zeros_like(p.data) for p in ps]
        self.v = [np.zeros_like(p.data) for p in ps]
        self.t = 0
    ...
    def step(self):
        ...
        for i, p in enumerate(self.ps):
            ...
            p.data -= lt * (self.m[i] / (np.sqrt(self.v[i]) + 1e-8) + self.wd * p.data)
            p.grad.fill(0)
