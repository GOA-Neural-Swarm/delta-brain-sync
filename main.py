            ...
            if isinstance(v, Tensor):
                ...
                for i in v:
                    p.extend(
                        ...
                        else [i] if isinstance(i, Tensor) else []
                    )
        return p
    ...
    def b(self, dy):
        return dy * (
            ...
            * (
                1
                + np.tanh(
                    np.sqrt(2 / np.pi) * (self.x + 0.044715 * np.power(self.x, 3))
                )
            )
        )
...
class MultiHeadAttention(Module):
    ...
    def b(self, dy):
        ...
        dv = (
            np.einsum("bsht,bshd->bthd", self.p, dy)
            .reshape(b, s, -1, self.g, self.hd)
            .sum(3)
        )
        return (
    ...
    def b(self, dy):
        ...
        for i in range(self.n):
            if self.cache[i] is not None:
                ...
                dx[m] += self.experts[i][0].b(
                    self.experts[i][1].b(
                        self.experts[i][2].b(dyf[m] * self.w[m, ps][:, None])
                    )
                )
