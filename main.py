...
class RN:
    ...
    def f(self, x):
        ...
        self.i = 1/np.sqrt(self.v + self.e)
        self.n = x * self.i
...
class At:
    ...
    def b(self, dy):
        ...
        da = self.p * (dp - np.sum(self.p * dp, -1, keepdims=True)) * self.s
        ...
        return self.wq.b(dqr.reshape(b,s,-1)) + self.wk.b(dk.reshape(b,s,-1)) + self.wv.b(dv.reshape(b,s,-1))

class ME:
    def __init__(self, d, n=4, k=2):
        self.d, self.n, self.k = d, n, k
        self.gate = L(d, n)
        self.w1 = [T(np.random.normal(0, np.sqrt(2/(d+d*2)), (d, d*2))) for _ in range(n)]
        self.w2 = [T(np.random.normal(0, np.sqrt(1/d), (d*2, d))) for _ in range(n)]
        self.sw = [SG() for _ in range(n)]
    def f(self, x):
        sh = x.shape
        x = x.reshape(-1, self.d)
        g = self.gate.f(x)
        p = np.exp(g - g.max(-1, 1, 1))
        p /= p.sum(-1, 1)
        self.ix = np.argsort(p, -1)[:, -self.k:]
        self.wt = np.take_along_axis(p, self.ix, -1)
        self.wt /= self.wt.sum(-1, 1) + 1e-12
    ...
    def b(self, dy):
        ...
        return (dx + self.gate.b(dg - np.mean(dg, -1, 1))).reshape(dy.shape)

class Blk:
    def __init__(self, d):
        self.n1, self.at = RN(d), At(d)
        self.n2, self.me = RN(d), ME(d)
    ...
    def b(self, dy):
        ...
        dm, dx1 = dy * self.p.d, dy * (1 - self.p.d)
        dx1 += self.n2.b(self.me.b(dm))
        return dx1 + self.n1.b(self.at.b(dx1))

class Model:
    def __init__(self, di, dm, do, depth=2):
        self.emb, self.bl = L(di, dm), [Blk(dm) for _ in range(depth)]
        self.fn, self.hd = RN(dm), L(dm, do)
        self.ps = self._gp()
    def _gp(self):
        p = []
        def w(o):
            if isinstance(o, T): p.append(o)
                ...
                for v in o.__dict__.values():
                    if isinstance(v, list): [w(i) for i in v]
                    else: w(v)
        w(self); return p
    def f(self, x):
...
class Opt:
    def __init__(self, ps, lr=1e-3, wd=0.01):
        ...
        self.m, self.v = [np.zeros_like(p.d) for p in ps], [np.zeros_like(p.d) for p in ps]
    def step(self):
        self.t += 1
...
def train():
    ...
    m, opt = Model(D, 128, C), Opt(Model(D, 128, C).ps, 3e-3) # Fixed initialization logic
    m.ps = opt.ps # Ensure parameters sync
    for e in range(E):
