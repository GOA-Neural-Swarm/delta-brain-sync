...
class O:
 ...
 def s(self,g):
  ...
  for i,p in enumerate(self.p):p*=.99998;self.m[i]=.9*self.m[i]+.1*g[i];self.v[i]=.999*self.v[i]+.001*g[i]**2;p-=a*self.m[i]/(self.v[i]**.5+1e-8)
class LN:
 def __init__(self,d):self.g,self.b=np.ones((1,d),f32),np.zeros((1,d),f32)
 def f(self,x):self.h=(x-x.mean(-1,keepdims=1))/(s:=np.sqrt(x.var(-1,keepdims=1)+1e-5));self.s=s;return self.g*self.h+self.b
 def bk(self,d):
  self.dg,self.db=(d*self.h).sum(0,keepdims=1),d.sum(0,keepdims=1);dx=d*self.g
...
class EN:
 ...
 def bk(self,d):
  ...
  self.u.s(g)
def train():
 X,Y,m,r=np.random.randn(100,784).astype(f32),np.random.randint(0,10,100),EN(),range(100)
 for e in range(101):
  z=m.f(X);p=(ex:=np.exp(z-z.max(1,keepdims=1)))/ex.sum(1,keepdims=1);l=-np.log(p[r,Y]+1e-10).mean();dl=p.copy();dl[r,Y]-=1;m.bk(dl/100)
