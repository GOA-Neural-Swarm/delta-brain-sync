...
class Mod:
    def __init__(self, di, dm, do):
        self.eb, self.bl, self.fn, self.hd, self.ps = L(di, dm), [B(dm) for _ in range(2)], N(dm), L(dm, do), []
        self._g(self)
    def _g(self, o):
    ...
    def b(self, dy):
        dy = self.fn.b(self.hd.b(dy)); db = np.zeros((dy.shape[0], 1, dy.shape[1])); db[:, -1, :] = dy
