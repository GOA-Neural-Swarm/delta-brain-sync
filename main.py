...
class N:
 def __init__(self,d):self.w,self.b=n.ones((1,d)),n.zeros((1,d))
...
class L:
 def __init__(self,i,o):self.w,self.b=n.random.randn(i,o)*(2/i)**.5,n.zeros((1,o))
...
class M:
 def __init__(self,i=784,h=128,o=10,c=3):
  self.l=[L(i,h),*[B(h) for _ in range(c)],L(h,o)];self.y=[y for x in self.l for y in getattr(x,'l',[]) if hasattr(y,'w')]+[x for x in self.l if hasattr(x,'w')]
...
m,U,I=M(),100,n.arange(100)
for e in range(101):
 z=m.f(X);v=n.exp(z-z.max(1,**k));p=v/v.sum(1,**k);ls=-n.log(p[I,Y]+1e-9).mean();d=p.copy();d[I,Y]-=1;m.b(d/U)
