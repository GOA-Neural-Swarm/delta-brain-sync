...
class GroqMoE:
    ...
    def forward(self, x):
        ...
        self.indices, self.probs, self.gate_probs = top_k_indices, top_k_probs, probs
        out = np.zeros_like(x_flat)
        self.ex_ctx = [[] for _ in range(self.num_experts)]
        ...
        for k in range(self.top_k):
            ...
            for i in range(self.num_experts):
                ...
                self.ex_ctx[i].append((mask, h, act, e_out, p[mask]))
    ...
    def backward(self, dout):
        ...
        for i in range(self.num_experts):
            for mask, h, act, e_out, p in self.ex_ctx[i]:
...
class SovereignBlock:
    ...
    def backward(self, dout):
        ...
        dout_norm2 = self.norm2.backward(dm)
        dout_res2 = dout + dout_norm2
        da = self.attn.backward(dout_res2)
        dout_norm1 = self.norm1.backward(da)
        return dout_res2 + dout_norm1
...
def train():
    X, y = get_data(4096)
    model = OMEGA_ASI_V9(h_dim=128, depth=2)
    params = model.get_params()
    opt = Lion(params, lr=1e-4)
    bs = 64
    print("OMEGA-ASI V9 | SOVEREIGN ARCHITECT | GQA-MOE HYBRID")
    for ep in range(50):
        ...
        for i in range(0, len(X), bs):
            ...
            opt.step()
        print(
            f"EP:{ep:02d} | LOSS:{l_sum/len(X):.4f} | ACC:{a_sum/len(X):.4f} | {len(X)/(time.time()-t0):.1f} samples/s"
        )
