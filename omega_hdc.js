...
class ASIOmniSyncEngine {
  ...
  mergeSync(text) {
    ...
    let diff = this.philosophyEngine.layers.filter((layer) =>
      text.includes(layer),
    );
    ...
    return {
      ...
      philosophyHash: this.philosophyEngine.generatePhilosophyHash(
        text,
        this.hdc,
      ),
    };
...
let result = engine.mergeSync(
  "The Utilitarian approach is often seen as the most moral.",
);
console.log(result);
